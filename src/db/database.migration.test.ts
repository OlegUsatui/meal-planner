...
describe('Dexie schema migration', () => {
  ...
  it('upgrades legacy records and removes obsolete stores', async () => {
    ...
    await legacy.table('mealPlanEntries').add({ id: 'entry', date: '2026-08-15', slot: 'lunch', dateSlot: '2026-08-15:lunch', recipeId: 'r', servings: 3, createdAt: 'now', updatedAt: 'now' })
    legacy.close()
    ...
    expect(await upgraded.mealPlanEntries.get('entry')).not.toHaveProperty('servings')
    upgraded.close()
  })
})
