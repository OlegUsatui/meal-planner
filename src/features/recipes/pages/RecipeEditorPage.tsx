...
export function RecipeEditorPage() {
  const { recipeId } = useParams(); const navigate = useNavigate(); const recipes = useRecipeRepository(); const productsRepo = useProductRepository(); const isAdmin = useOptionalAuth()?.isAdmin ?? false; const [products, setProducts] = useState<Product[]>([]); const [recipe, setRecipe] = useState<Recipe>(); const [error, setError] = useState<string>(); const editing = Boolean(recipeId)
  useEffect(() => { void productsRepo.list({ includeArchived: isAdmin }).then(setProducts); if (recipeId) void recipes.get(recipeId).then(setRecipe).catch(() => setError('Не вдалося знайти рецепт')) }, [isAdmin, productsRepo, recipeId, recipes])
