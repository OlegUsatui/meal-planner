...
export function RecipesPage() {
  ...
  useEffect(() => { let active = true; setLoading(true); repository.list(query).then((value) => active && setRecipes(value)).finally(() => active && setLoading(false)); return () => { active = false } }, [repository, query])
  ...
  return <section className="page recipes-page"><header className="page-header"><div><p className="eyebrow">Страви та категорії</p><h1>Рецепти</h1><p className="page-intro">Зберігайте улюблені страви й знаходьте їх за прийомом їжі та тематичним розділом.</p></div><Link className="button button-primary" to="/recipes/new">+ Новий рецепт</Link></header>
    ...
    {loading ? <div className="loading-panel">Завантажуємо рецепти…</div> : visible.length ? <div className="recipe-grid">{visible.map((recipe) => <RecipeCard recipe={recipe} key={recipe.id} />)}</div> : <div className="empty-state"><div className="empty-illustration">🍲</div><p className="eyebrow">Ваша книга рецептів</p><h2>{query || section !== 'all' ? 'Нічого не знайдено' : 'Створіть перший рецепт'}</h2><p>{query || section !== 'all' ? 'Змініть пошук або категорію.' : 'Додайте фото, категорії, інгредієнти та спосіб приготування.'}</p><Link className="button button-primary" to="/recipes/new">Створити рецепт</Link></div>}
  </section>
}
...
function RecipeCard({ recipe }: { recipe: Recipe }) {
  ...
  return <Link className="recipe-card" to={`/recipes/${recipe.id}`}><img src={url} alt="" /><div><p className="eyebrow">{formatPreparationTime(recipe.preparationTimeMinMinutes, recipe.preparationTimeMaxMinutes) ?? 'Час не вказано'}</p><h2>{recipe.name}</h2><p>{labels.length ? labels.slice(0, 2).join(' · ') : 'Без категорії'}</p></div></Link>
}
