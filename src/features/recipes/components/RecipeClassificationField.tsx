...
export function RecipeClassificationField({ value, onChange }: { value: RecipeClassification[]; onChange: (value: RecipeClassification[]) => void }) {
  ...
  return <fieldset className="recipe-classification-field"><legend>Категорії рецепту</legend><p className="field-hint classification-helper">Спочатку виберіть прийом їжі, потім одну чи кілька релевантних підкатегорій.</p>
    <div className="classification-meal-types" aria-label="Прийом їжі">{recipeMealTypes.map((item) => <button type="button" key={item.value} aria-pressed={mealType === item.value} onClick={() => setMealType(item.value)}>{item.label}{value.some((entry) => entry.mealType === item.value) && <span aria-hidden="true"> · {value.filter((entry) => entry.mealType === item.value).length}</span>}</button>)}</div>
