...
export function RecipePickerDialog({ date, slot, recipes, initialRecipeId, initialServings = 1, onClose, onSave }: Props) {
  ...
  return <div className="planner-dialog-backdrop" onMouseDown={backdrop}><div className="planner-dialog picker-dialog" role="dialog" aria-modal="true" aria-labelledby="picker-title" ref={dialogRef}>
    <form onSubmit={submit}><header className="planner-dialog-header"><div><p className="eyebrow">{slotLabel} · {parseLocalDate(date).toLocaleDateString('uk-UA', { day: 'numeric', month: 'long' })}</p><h2 id="picker-title">{initialRecipeId ? 'Замінити страву' : 'Додати страву'}</h2></div><button type="button" className="dialog-close" aria-label="Закрити" onClick={onClose}>×</button></header>
      {eligible.length ? <><div className="picker-filter-row"><label className="picker-search"><span className="sr-only">Пошук рецептів</span><input type="search" placeholder="Знайти рецепт…" value={query} onChange={(event) => setQuery(event.target.value)} /></label><label className="picker-category"><span>Підкатегорія</span><select value={subcategory} onChange={(event) => setSubcategory(event.target.value)}><option value="">Усі підкатегорії</option>{categories.map((item) => <option value={item.subcategoryId} key={item.subcategoryId}>{item.label}</option>)}</select></label></div><div className="picker-recipes">{filtered.map((recipe) => <button type="button" key={recipe.id} className={`picker-recipe ${selectedId === recipe.id ? 'selected' : ''}`} aria-pressed={selectedId === recipe.id} onClick={() => setSelectedId(recipe.id)}><RecipeImage blob={recipe.image.blob} url={recipe.image.url} alt="" className="picker-recipe-image" /><span><strong>{recipe.name}</strong><small>{recipe.classifications.length ? recipe.classifications.filter((item) => item.mealType === slot).map((item) => getRecipeSubcategory(item.subcategoryId)?.label).filter(Boolean).join(' · ') : 'Без категорії'}</small></span></button>)}</div>{!filtered.length && <p className="picker-empty">За цим запитом рецептів немає.</p>}<div className="picker-footer"><label>Кількість порцій<input type="number" min="1" max="99" value={servings} onChange={(event) => setServings(Number(event.target.value))} /></label><button className="button button-primary" disabled={pending || !selectedId}>{pending ? 'Зберігаємо…' : 'Додати до плану'}</button></div></> : <div className="picker-empty"><h3>Немає рецептів для цього прийому їжі</h3><p>Додайте відповідну категорію до рецепту або створіть новий.</p></div>}
    </form></div></div>
}
