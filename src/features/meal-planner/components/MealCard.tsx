...
export function MealCard({ entry, recipe, readOnly, onOpen, onReplace, onRemove }: { entry: MealPlanEntry; recipe: Recipe; readOnly: boolean; onOpen: (trigger: HTMLElement) => void; onReplace: () => void; onRemove: () => void }) {
  ...
  return <article className="meal-card">
    ...
    {!readOnly && <div className="meal-card-menu-wrap"><button type="button" className="meal-menu-trigger" aria-label={`Дії для ${recipe.name}`} aria-expanded={menuOpen} onClick={openMenu}>⋯</button>{menuOpen && <div className="meal-card-menu"><button type="button" onClick={() => { setMenuOpen(false); onReplace() }}>Замінити</button><button type="button" className="danger" onClick={() => { setMenuOpen(false); onRemove() }}>Видалити</button></div>}</div>}
  </article>
}
