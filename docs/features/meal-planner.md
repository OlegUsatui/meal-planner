...
- `GET /api/meal-plan?from&to` — inclusive visible-week read. The selected `day|week` view is stored per authenticated user and restored when returning to `/plan`; an explicit `view` URL parameter takes precedence.
...
Two-level calendar: a seven-day date strip followed by one spacious selected-day panel in day mode, with an optional seven-column week mode from the view switcher. Desktop day mode uses the available main-content width as one editorial paper sheet rather than a narrow centered column; its 4:3 recipe images are prominent, titles and metadata have clear hierarchy, and the servings footer remains visually attached to each card. Day cards remain rich and show the recipe title, preparation time, category and available per-serving nutrition. Week cards use a compact vertical editorial layout: 4:3 image or illustrated placeholder, visible two-line recipe title, preparation time/category metadata, servings control and the existing replace/remove action menu. The calendar uses a paper grid, dashed pencil-like boundaries, and warm slot surfaces without changing the underlying meal actions. The week view uses summaries only; full recipe details are loaded only for the selected day or when opening a recipe, avoiding duplicate transient requests. Add/replace navigates to a dedicated selection page and returns to the same week/date after saving. The selection page uses the same recipe-card grid and visual hierarchy as the Recipes catalogue; its fixed bottom action bar keeps the primary action visible without a duplicate selected-recipe summary panel.
...
Change week/day, switch between day/week views, return to today, add, replace, remove, change servings, open detail from a planned card or the add/replace selection card, search/filter on the selection page, retry a stale read, and recover a missing recipe.
